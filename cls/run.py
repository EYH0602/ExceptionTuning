...
def convert_examples_to_features(
    ...
    type_ids = source_type_ids + error_type_ids + [0] * padding_length
    pos_ids = source_pos_ids + error_pos_ids + [tokenizer.pad_token_id] * padding_length
    results.append(
        InputFeatures(
            source_tokens_f,
            source_ids_f,
            js["index"],
            int(js["label"]),  # no -1 since label start at 0 now
            type_ids,
            pos_ids,
        )
    )
    ...
    if args.need_raw == True:
        results = []
    # if there is no avalible error, add only the source
    if len(results) == 0:
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return (
            ...
            torch.tensor(self.examples[i].type_ids),
            torch.tensor(self.examples[i].pos_ids),
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    result = {"eval_loss": float(perplexity), "eval_acc": float(eval_acc)}
...
def main():
    ...
    parser.add_argument("--error_prompt", type=str, default="", help="error_prompt")
    parser.add_argument(
        "--need_raw",
        help='Whether need raw, True or False flag, input should be either "True" or "False".',
        type=ast.literal_eval,
        dest="need_raw",
    )
    parser.add_argument("--nohang", action="store_true", help="Whether no hang")
    parser.add_argument("--nocrash", action="store_true", help="Whether no crash")
    parser.add_argument(
        "--unite_crash_hang",
        action="store_true",
    ...
    parser.add_argument(
        "--unite_crash_hang_token",
        type=str,
        default="",
        help="united crash and hang token if needed",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--early_stop", type=int, default=4, help="early stop used for traninig"
    )
    parser.add_argument(
        "--n_class", type=int, default=1000, help="number of class for problems"
    )
